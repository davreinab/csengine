...
## 2026-09-05 · David + Claude · (este push)
- ⚠️ **"Certified Partner" → "Shopify Plus Partner" en todo el sitio** (decisión de David): badge oficial en `img_d/partners/shopify-plus-partner.svg` (+variante `-white.svg`); `shopify-certified-partner.png` **eliminado**. Metadescriptions/OG de home y shopify actualizadas; `lastmod` al día. **OJO dato a validar con cliente:** según la página del programa de Shopify, el programa Plus Partner clásico se retiró (dic. 2024) y sus marcas se descontinuaron (dic. 2025); el programa actual tiene niveles Registered/Select/Plus/Premier/Platinum. Confirmar estatus real y uso del badge.
- **shopify.html:** sello en el hero a la IZQUIERDA de la entradilla (nuevo `.intro-row` + `.hero-seal`, tokens `--hero-seal-w(-sm)`), entradilla mencionando el reconocimiento en `<b>`. La pastilla blanca `.partner-featured` sustituida por variante **`--split` sin pastilla**: H2 sticky en scroll a la izq. (estático en móvil), sello 320px (`--partner-feat-logo-lg`) + 3 párrafos a la der. (`.pf-body`). Copy basado en refs (rock.et, valtech, wuala, ecomm360) sin inventar cifras ni servicios.
- **Footer (11 páginas):** badge Shopify Plus Partner blanco bajo el claim (nuevo `.foot-plus`, token `--footer-plus-w` 176px). **Bug fix:** la pastilla "★ 20 años" estaba sin estilo (`.foot-claim .seal` no matcheaba — el span está fuera del `<p>`); ahora `.foot-top .seal`. Verás la pastilla lima "nueva": es la corrección.
- **Móvil:** TODOS los `.btn` a ancho completo en ≤960px (regla genérica; sustituye a la específica de `.hero-ctas`). "Shopify Plus Partner" en negrita en párrafos (home + shopify). Lint 0/0.
